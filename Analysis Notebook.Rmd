...
View(daily_individual_reg_data)

indiv_arrival_reg_data <-read.csv(paste0(analysis_datapath,"individual_per_quarter.csv"),
                                   colClasses = c("control_number" = "character"))
indiv_arrival_reg_data <- indiv_arrival_reg_data[, -1] %>% 
  mutate(period_group = ymd(period_group)) %>% 
  ungroup()

View(indiv_arrival_reg_data)
...
View(monthly_indiv_arrival_data)
# facility_arrival_reg_data <-
```
...
daily_individual_reg_data_quarter
# 
# pattern <- "_[a=z]"
# extracted_label <- gregexpr(pattern, toString(data), value = TRUE)
