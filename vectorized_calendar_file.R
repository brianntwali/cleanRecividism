...
increment_date <- function(date_vector) {
  date_vector <- as.Date(date_vector, format = "%m%d%Y")
  incremented_dates <- date_vector + 1
  return(format(incremented_dates, "%m%d%Y"))
}
...
compare_dates <- function(date_one_vector, date_two_vector) {
  date_one_vector <- as.Date(strptime(date_one_vector, format = '%m%d%Y'))
  date_two_vector <- as.Date(strptime(date_two_vector, format = '%m%d%Y'))
  return(date_one_vector > date_two_vector)
}

# Tests for function
compare_dates('01082020', '02012028')
compare_dates('12082028', '02012028')
...
get_prev_status_date <- function(ID_vector, max_date_vector) {
  max_date_vector <- as.Date(max_date_vector, format = '%m%d%Y')
  
  # Assuming cc_counts_dt_sorted_desc is precomputed
  prev_status_dates <- sapply(1:length(ID_vector), function(i) {
    status_date <- cc_counts_dt_sorted_desc[(control_number == ID_vector[i] & status_date < max_date_vector[i]), status_date][1]
    if (is.na(status_date)) return(NA)
    return(format(status_date, '%m%d%Y'))
  })
  
  return(prev_status_dates)
}
